...
namespace eyestep {
	...
	label::label(char* x, UINT y, USHORT size) {
		name[0] = '\0';
		strcpy_s(name, x);
		address = y;
		if (address == 0) {
	...
	inst write(int address, std::string str, std::vector<label>labels) {
		inst i = inst();
		BYTE bytes[16];
